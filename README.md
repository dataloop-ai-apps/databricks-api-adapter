...
This repository contains the code for DDOE model adapters that invoke models served in Databricks via their API. To use this model adapter, it is needed to have a Databricks workspace and a model serving endpoint in the workspace.
...
Once you install this model adapter in a project in the DDOE platform, visit its model page to finish its configuration. There you will need to change the two fields marked with the red rectangles here:
...
you can keep the ```model_entity``` value. Save the configurations and the service should be ready to be used for predictions in the DDOE platform.
